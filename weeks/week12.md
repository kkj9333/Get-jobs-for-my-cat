...
## 前置知识
### 流形
流形是一个数学对象，通过一个低维曲面或子空间来近似一个高维空间，捕捉数据的基本属性和结构。当处理复杂数据集时，特征可能相互依赖或相关，流形的概念特别有用。
### 流形学习
#### 高维空间有冗余，低维空间没冗余
流形可以作为一种数据降维的方式。
...
### 深层特征的特征流形
深度学习中特征流形更加复杂，非线性化。非线性流形学习的关键困难是无组织的降维数据。PCA或多维尺度化（MDS），都不能重建目标空间中的底层结构信息。  
对非线性深度特征的流形学习的目标是在不明确知道h（·）的情况下，从相应的特征fi或h（vi）重构vi。
假设深度学习特征空间F是R^m的一个子集，特征fi∈F具有未知的生成函数h (v)，v∈R^d，通过训练深度学习模型，我们收集了一组N个m-维特征向量F = [f1，f2，···，fN ]，fi∈R^m，且fi无噪音地从h(vi)产生。

对于非线性流形，假设函数h足够光滑，利用一阶泰勒展开式，并定义一个小的邻域vˆ，使{||vˆ−v||2 <ϵ}，ϵ小。让hv = [h1(v)，···，hm(v)]⊤，于是有h（·）的局部仿射近似：
![image](https://user-images.githubusercontent.com/51207072/232984911-e4775858-6003-4436-b029-c9a5e2809d20.png)
其中倒三角vh(V^t)表示雅可比矩阵。
...
- 上下文干扰（Interference Among Contexts）。上下文划分法得到的上下文之间的干扰，具体来说，当智能体在其他上下文下学习时，我们在游戏的不同上下文中测量TD错误的Huber损失，然后记录智能体学习前后损失的相对变化。如上图结果表明，在任何情境下的长期训练都可能导致在所有其他情境下的负泛化，即使是在这样简单的RL任务CartPole-v0中（补充材料的附录A-C中运行结果也支持这个结论）。
